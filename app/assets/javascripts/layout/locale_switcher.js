(function() {
  ...
  App.LocaleSwitcher = {
    initialize: function() {
      $(".locale-switcher").each(function() {
        ...
        $(this).on("show.zf.dropdownMenu", function() {
          $(this).find("[aria-selected='true']")[0].scrollIntoView({ block: "nearest" });
        });

        $(this).find("ul ul").on("keydown", function(event) {
          App.LocationChanger.focusElementStartingWith(String.fromCharCode(event.which));
        });
      });
    },
    focusElementStartingWith: function(character) {
      ...
      var match = App.LocationChanger.firstStartingWith(active_item.nextAll(), character) ||
        App.LocationChanger.firstStartingWith(active_item.siblings(), character);
